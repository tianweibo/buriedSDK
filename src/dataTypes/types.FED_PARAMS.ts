...
import { ENUM_RUNTIME_ENV } from "./types.ENUM_RUNTIME_ENV";
export type FED_PARAMS = {
    //新增字段
    nick:string,      //wangwnag
    mix_nick:string,  //wnagwanao mi
    open_id:string,   
    phone:string,
    ouid:string,      //open user id 
    provider:string,   //
    open_type:number, //1正常数据也就是对接新埋点平台，2互动营销类的，3其他
    act_name:string
    // When	

    // event_uuid，server_time这两个前端不用传，由filebeat提供的
